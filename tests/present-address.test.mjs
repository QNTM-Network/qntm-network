 ...
 * `Object.keys(sections[view])[ordinal]` is silently wrong whenever the published predicate table
 * is a proper SUBSET of a view's declared sections.
 *
 * 2026-08-06 (job 1, "the last fourteen") CLOSED THE REAL CONFIG'S OWN LAST GAPS: `daily-work` (was
 * 1 of 5) and `daily-personal` (was 3 of 8) — the two surfaces this file used to cite as the trap's
 * live example — now publish EVERY declared section, and measured across the whole real config, 0
 * of 83 published views have a gap today. The trap `sectionAt` closes is still real (a fact about
 * the FUNCTION, never about which patterns happen to normalise on a given day), so section 2 below
 * now proves it with a small SYNTHETIC gap alongside the real, now-gapless `daily-work`/
 * `daily-personal` fixtures — both kept, because "every declared ordinal still addresses even with
 * no gap" is exactly as real a claim as "even with one".
 *
 * Four sections:
 *
 ...
 *      change) and REAL markdown transcribed from his vault — `inbox`, `daily-work` and
 *      `daily-personal` (all addressed correctly through `SECTION_ORDER`, gap or no gap today),
 *      plus a SYNTHETIC gap that keeps the trap itself concrete.
...
describe("2. sectionAt — the join, against the REAL declaration and REAL vault content", () => {
  ...
  describe("THE TRAP CASES — every declared ordinal addresses, not only the published one", () => {
    ...
    test("THE FAILING VERSION OF THIS TEST — indexing the published subset gets it wrong, SYNTHETIC fixture", () => {
      ...
      // claim rather than only prose. `Object.keys(sections[view])[ordinal]` on a hand-built view.
      //
      // 2026-08-06 (job 1, "the last fourteen") RETIRED THE REAL daily-work EXAMPLE THIS TEST USED
      // to demonstrate the trap: measured across the WHOLE real config, 0 of 83 published views
      // have a published subset smaller than their declared order today (the "precondition" test
      // above proves it for THIS view specifically). `sectionAt` still MUST index the full
      // `sectionOrder`, never the published subset — that is a fact about the FUNCTION, not about
      // today's config — so this test now proves it with a small hand-built gap instead, the same
      // "small fixture" register section 1 of this file already uses.
      const GAP_ORDER = { "synthetic-view": ["alpha", "beta", "gamma", "delta"] };
      const GAP_SECTIONS = { alpha: {}, gamma: {}, delta: {} }; // beta: declared, never published
      const GAP_SOURCE = ["## Alpha", "## Beta", "## Gamma", "## Delta"].join("\n");

      const naive = Object.keys(GAP_SECTIONS);
      assert.equal(naive[0], "alpha", "ordinal 0 happens to agree");
      assert.equal(
        naive[1],
        "gamma",
        "ordinal 1 ('beta') is where the naive version silently returns the WRONG section",
      );
      assert.notEqual(
        naive[1],
        sectionAt(GAP_SOURCE, 1, "synthetic-view", GAP_ORDER),
        "the naive index and the real answer for ordinal 1 must disagree — that IS the trap",
      );
      assert.equal(sectionAt(GAP_SOURCE, 0, "synthetic-view", GAP_ORDER), "alpha");
