...
ENGINE_REVISION = monorepo_config.engine_revision(ENGINE_SRC)
...
def main() -> int:
    ...
    # ── AND THE CHECK EVERY ONE ABOVE IS BLIND TO: *WHICH* ENGINE WAS READ ────────────
    #
    # Every refusal above examines the CONTENT of what was read. None of them examines
    # the REVISION. On 2026-08-14 this script read a shared trunk clone that had been
    # sitting on a feature branch for three hours, captured an engine that predated the
    # change under test, printed "10 sheet keys" and exited 0. The downstream gate stayed
    # green through a merged engine change, because "the engine has not changed" and "I
    # read an engine from three days ago" spell exactly the same output.
    refusal = monorepo_config.capture_refusal(ENGINE_REVISION, ENGINE_SRC)
    if refusal:
    ...
    fixture = {
        ...
        # WHICH ENGINE THIS IS, not only what it allows — the `...Source` discipline
        # `compile-resolution.mjs` keeps for every answer it publishes, applied to the
        # capture. Without it a stale fixture and a current one are indistinguishable.
    ...
    print(
        ...
        f"  engine: {ENGINE_REVISION['branch']} @ {str(ENGINE_REVISION['sha'])[:12]}"
        + ("  (QNTM_MONOREPO_ENGINE_SRC)" if ENGINE_REVISION["overridden"] else "")
    )
    return 0
