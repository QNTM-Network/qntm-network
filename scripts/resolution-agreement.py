...
# STEP "the engine's own default ordering, made explicit" (2026-08-04) — the SIXTH comparison this
# script makes, and the only one that is NOT config-dependent at all: `_DEFAULT_ORDERING`/
# `_PRIORITY_RANK` are module-level constants in the RENDERER, read here by LIVE IMPORT (never
# transcribed) so `scripts/compile-resolution.mjs`'s own JS copy of the same two facts
# (`ENGINE_DEFAULT_ORDERING`/`ENGINE_PRIORITY_RANK`) can be pinned against the engine's actual
# values rather than against a comment claiming what they are.
from qntm_md.render.section_builder import (  # noqa: E402
    _DEFAULT_ORDERING as ENGINE_DEFAULT_ORDERING,
    _PRIORITY_RANK as ENGINE_PRIORITY_RANK,
)
...
def main() -> int:
    ...
    # ── THE ENGINE'S DEFAULT ORDERING (2026-08-04) — LIVE IMPORT, NOT CONFIG-DEPENDENT ──────────
    #
    # `ENGINE_DEFAULT_ORDERING`/`ENGINE_PRIORITY_RANK` (imported above) are the same objects
    # `section_builder.py`'s own `_section_order_key` sorts by — not a re-derivation, the actual
    # runtime values. Compared here against what THIS run's `resolution["defaultOrdering"]`/
    # `["priorityRank"]` publish, and REFUSING (exit 2) rather than writing a fixture that would
    # let the browser's copy silently drift from the engine's own.
    engine_default_ordering = [
        {"field": entry.field, "direction": entry.direction} for entry in ENGINE_DEFAULT_ORDERING
    ]
    published_default_ordering = resolution.get("defaultOrdering")
    if published_default_ordering != engine_default_ordering:
        print(
            f"REFUSING: published defaultOrdering={published_default_ordering!r} != engine "
            f"_DEFAULT_ORDERING={engine_default_ordering!r}",
            file=sys.stderr,
        )
        return 2
    published_priority_rank = resolution.get("priorityRank")
    if published_priority_rank != dict(ENGINE_PRIORITY_RANK):
        print(
            f"REFUSING: published priorityRank={published_priority_rank!r} != engine "
            f"_PRIORITY_RANK={dict(ENGINE_PRIORITY_RANK)!r}",
            file=sys.stderr,
        )
        return 2
    # POSITIVE CONTROLS — a comparison of two EMPTY/trivial structures would pass even if the
    # import above silently resolved to nothing.
    if len(engine_default_ordering) == 0:
        print("REFUSING: _DEFAULT_ORDERING imported as empty — this comparison would be vacuous", file=sys.stderr)
        return 2
    if len(set(ENGINE_PRIORITY_RANK.values())) == len(ENGINE_PRIORITY_RANK):
        print(
            "REFUSING: no two priority names share a rank in the imported _PRIORITY_RANK — the "
            "operator's own config comment ('normal'/'medium' share rank 2) could not be verified "
            "against this measurement, which is a positive control this script depends on",
            file=sys.stderr,
        )
        return 2
    ...
    fixture = {
        "note": (
            ...
            "candidate against qntm_md.grammar.node_type_form.node_type_forms, plus (2026-08-04) "
            "the engine's own default ordering, imported LIVE from "
        ...
        "priorityRank": dict(ENGINE_PRIORITY_RANK),
    }
