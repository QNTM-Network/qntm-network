 ...
 *   node scripts/generate-resolution-declaration.mjs --check --require-config  ... and FAIL if there is no config to check
 */
...
async function main() {
  ...
  if (args.check) {
    if (JSON.stringify(current.resolution) === JSON.stringify(withoutLedger(resolution))) {
    ...
    // `current.resolution?.dropped ?? {}` — the drops read back out of presentation.json, which is
    ...
    const before = readLedger(presentationPath, 'resolution');
  ...
  writeFileSync(
    presentationPath,
    JSON.stringify({ ...current, resolution: withoutLedger(resolution) }, null, 2) + "\n",
  );
  ...
  const ledgerPath = writeLedger(presentationPath, 'resolution', resolution.dropped);
